...
def test_pre_train_model(dataset):
    ...
    pretrained_cnn = PretrainedModel('PDGAN/Face/2_net_EN.pth', 'PDGAN/Face/2_net_DE.pth')
...
if __name__ == "__main__":
     # Define dataset
    dataset_path = 'CelebA/CelebA/Img/img_celeba.7z/img_celeba'
    mask_path = 'mask/testing_mask_dataset/'
    encoder_path = 'PDGAN/Face/2_net_EN.pth'
    decoder_path = 'PDGAN/Face/2_net_DE.pth'
    dataset = Dataset('CelebA/CelebA/Img/img_align_celeba', True, 'mask/testing_mask_dataset')
    img_samples = []
    mask_samples = []
    for i in range(5):
    ...
    model_kv = torch.load('model/generator.pth')
